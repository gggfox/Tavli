# ADR-010: WhatsApp Assistant as a Twilio-Backed LLM First Responder
...
**Domain objects.** A `whatsappChannels` row maps one WhatsApp sender number to one `Restaurant`, which is how an inbound message is routed. A `Conversation` is the thread between one customer phone and one channel. `whatsappMessages` is an append-only log deduped on Twilio's `MessageSid`. (Superseded by [ADR-012](./012-one-tavli-whatsapp-number.md): Tavli is now the sender on one shared number, so a channel row records that a restaurant is _enabled_ with a short code, and a `Conversation` is per (customer phone, `Restaurant`).)
...
- [ADR-011: Phone-Number Identity for Customer Reservation Writes](./011-whatsapp-customer-reservation-writes.md) — supersedes the read-only scope
...
| 2026-07-25 | Jorge Almazan | Read-only scope superseded by ADR-011: the assistant can now request and cancel reservations. The runtime split, domain objects and formatting decisions here still stand.                                                                                                                                                                               |
| 2026-08-23 | Tavli team    | Routing superseded by ADR-012. Meta retired the On-Behalf-Of WABA model on 29 Sep 2025, so the per-restaurant senders this ADR assumed are not obtainable on a pilot's timeline; Tavli sends, and a per-restaurant short code in a `wa.me` deep link routes instead of the Twilio "To". The runtime split and the formatting decisions here still stand. |
