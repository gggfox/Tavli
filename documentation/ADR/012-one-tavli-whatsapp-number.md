...
| 2026-08-24 | Tavli team | Consent _replies_ are metered. Recording a STOP stays unconditional, but its confirmation — like the unroutable guidance, a fixed reply to a number that proved only a Twilio signature — now spends the phone's inbound and outbound budgets and stops at the platform ceiling. Re-opting in is refused once the phone's inbound cap is spent, which is what bounds STOP/START alternation: each transition otherwise wrote a permanent `allEvents` row for free. |
