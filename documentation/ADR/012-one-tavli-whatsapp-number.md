...
**Enabling is platform-admin only** — `getCurrentUserId` then `requireAdminRole`, the same gate as `convex/featureFlags.ts`. Not self-serve: every enabled restaurant is spend on Tavli's own Twilio and OpenRouter accounts. (TAVLI-95 later added the subscription gate — an enrolled restaurant whose platform subscription lapses stops getting model turns — but enabling stays admin-only: the gate bounds a lapse, it does not make the spend self-serve.) Reading is wider — a manager sees the link, the code and the QR, because they are the person who prints it.
...
| 2026-08-24 | Tavli team | Production blockers (TAVLI-95): per-phone consent/opt-out (STOP/BAJA/ALTO ↔ START/ALTA, above every budget; conversations record the opt-in), 90-day message retention, and two new inbound gates — a deleted/deactivated restaurant answers with honest fixed copy and is skipped by routing, and a restaurant whose platform subscription lapsed gets fixed copy instead of model turns.                                                                                                                                                                                                                                                                                                                                       |
| 2026-08-24 | Tavli team | Consent _replies_ are metered — like the unroutable guidance, a fixed reply to a number that proved only a Twilio signature. Recording a STOP stays unconditional, and so does the confirmation that says how to return: the phone most likely to send STOP is the one already deep into its inbound cap. That reply is still billed, so it stops at the platform ceiling and at the phone's outbound cap; it does not wait for inbound headroom. STOP/START alternation is bounded one step higher instead — re-opting in is refused once the phone's inbound cap is spent, so past the cap a phone buys at most one more confirmation and then stays opted out, rather than writing a permanent `allEvents` row per free send. |
| 2026-09-17 | Tavli team | Inactive is not deleted (TAVLI-107). A paused restaurant's code still routes and the thread is recorded; its gate answers with "not available right now" copy, while a deleted restaurant keeps looking like an unknown code. Cold start prefers active restaurants and binds to a paused one only when it is the sole restaurant in the phone's history. Enabling the assistant at an inactive restaurant is refused (`ERROR_RESTAURANT_INACTIVE`); the assistant's off-state follows the restaurant's `isActive` and is never written to the channel; the public deep link hides while the restaurant is inactive.                                                                                                             |
