...
10. **The machine-identity client secret exists in two places and will drift.** CI reads it from GitHub Actions; the container reads its own copy from the Dokploy app env. It is one-time-view, and it can also carry a **TTL** or a **max-uses** limit — so it can stop working without anyone touching it. Any of those makes every new container 401 and exit(1) at boot in ~400ms while the previous one keeps serving. **Rotating is always a two-place change, and a replacement should be created with TTL `0` / max uses `0`.**
