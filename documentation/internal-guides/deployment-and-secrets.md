...
10. **The machine-identity client secret exists in two places and will drift.** CI reads it from GitHub Actions; the container reads its own copy from the Dokploy app env. The Universal Auth client secret is one-time-view, so rotating it for CI silently leaves Dokploy on the old value — every new container then 401s and exits(1) at boot in ~400ms while the previous one keeps serving. **Rotating this secret is always a two-place change.**
