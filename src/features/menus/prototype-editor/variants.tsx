...
function SaveButtons({
	...
	return (
		<div className="flex items-center gap-2">
			<span className="mr-auto text-[11px] text-faint-foreground">
				{d.dirty ? (
					<span className="flex items-center gap-1.5 text-warning">
						<span className="h-1.5 w-1.5 rounded-full bg-warning" /> Cambios sin guardar
					</span>
				) : (
					"Esc para cerrar · ⌘↵ para guardar"
				)}
			</span>
			<button
				type="button"
				...
				className="rounded-lg px-3 py-2 text-sm hover-btn-secondary"
			>
				Cancelar
			</button>
			<button
				...
				className={`rounded-lg px-4 py-2 text-sm font-medium disabled:opacity-40 ${extra ? "hover-btn-secondary" : "hover-btn-primary"}`}
			>
				Guardar
			</button>
			{extra}
	...
	return (
		<div className="fixed inset-0 z-40 flex items-center justify-center p-4">
			...
			<div
				...
				className="relative flex max-h-[92dvh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-lg)]"
			>
				<header className="flex items-center gap-2 border-b border-border px-5 py-3">
					...
					<div className="flex">
						<IconButton label="Producto anterior" onClick={() => go(prev)}>
							<ChevronLeft size={18} className={prev ? "" : "opacity-30"} />
						</IconButton>
						<IconButton label="Producto siguiente" onClick={() => go(next)}>
							<ChevronRight size={18} className={next ? "" : "opacity-30"} />
						</IconButton>
					</div>
				...
				<div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-5 md:grid-cols-[16rem_1fr]">
					<ImageWell d={d} />
					<Fields d={d} />
				</div>
				<footer className="border-t border-border px-5 py-3">
						...
						extra={
							next ? (
								<button
									...
									className="flex items-center gap-1 rounded-lg px-4 py-2 text-sm font-medium hover-btn-primary"
								>
