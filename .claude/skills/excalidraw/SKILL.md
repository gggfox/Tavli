...
## File-Based Generation (writing .excalidraw JSON directly, no live canvas)

When the MCP canvas is unavailable you can still apply this skill by generating
`.excalidraw` files programmatically. Three rules change — each learned the hard
way on a dense 38-entity ERD:

### 1. Bindings betray you in saved files

Editors recompute bound-arrow endpoints from `startBinding`/`endBinding`
(focus/gap) as soon as the arrow or a bound shape is touched — and elbow
arrows already on load. Programmatically authored focus/gap values rarely
agree with your authored points, so bound arrows visibly snap away from the
border. For generated files:

- Compute exact endpoint geometry yourself; endpoints must land ON the box border
- **Omit `startBinding`/`endBinding` entirely** (and leave `boundElements: []`)
- Trade-off: dragging a box won't drag its arrows. For generated reference
  diagrams, regenerate from the script instead of hand-editing.

Related: never AUTHOR a text↔shape binding for a card title (a `containerId`
on the text, or the text listed in the shape's `boundElements`) — bound
...
A straight line between two distant boxes WILL cross unrelated boxes. Never
draw center-to-center diagonals. Route like a PCB:

- **Columns + gutters**: lay boxes out in columns; keep 100–120px empty gutters
  between columns; every vertical arrow run lives inside a gutter
- **Lanes**: give each arrow in a gutter its own x-offset (12–14px apart) so
  vertical runs never overlap
- **Corridors**: reserve an empty horizontal strip above (or below) all content
  for long-haul arrows; descend into the destination via a gutter — never drop
  through a column of boxes
- **Anchor slots**: when several arrows attach to the same box edge, spread the
  attachment points (~18px apart) so arrowheads don't stack
- **Adjacency first**: order columns (and stacking within them) so most related
  boxes end up same-column or adjacent-column; a parent directly above its child
  needs only a short straight vertical arrow
- **Prune ubiquitous edges**: a hub referenced by every box (tenant id, audit
  user) produces an unreadable mesh; drop those arrows and state the convention
  in the legend instead
- **Sharp corners**: multi-point arrows must use `"roundness": null` —
  `{"type": 2}` turns an orthogonal polyline into a loose curve that swings
  over neighboring boxes

### 3. Validate before shipping

- Run a segment-vs-rectangle intersection check: every arrow segment against
  every box except the arrow's own source/target. Zero crossings is the bar.
  With orthogonal routing all segments are axis-aligned, so the check is trivial.
- Then render and eyeball a screenshot. Render with the CURRENT
  `@excalidraw/excalidraw` package (the generation real editors bundle) — the
  legacy `@excalidraw/utils@0.1.2` mis-anchors multi-line text and will send
  you chasing phantom layout bugs.
- Check the render in both light and dark themes; author colors for light and
  let Excalidraw's dark filter invert.
