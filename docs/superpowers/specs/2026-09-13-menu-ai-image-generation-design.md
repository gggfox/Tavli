...
`MENU_AI_IMAGE_DAILY_LIMIT_PER_RESTAURANT = 150`,
`MENU_AI_IMAGE_GLOBAL_DAILY_LIMIT = 1000`,
...
`ERROR_AI_IMAGE_GENERATION_IN_PROGRESS`, `ERROR_AI_IMAGE_DAILY_LIMIT_REACHED`,
`ERROR_AI_IMAGE_PLATFORM_LIMIT_REACHED`, `ERROR_AI_IMAGE_CREDITS_EXHAUSTED`,
...
4. Spend: `consumeRateLimit` on `menu_ai_image:<restaurantId>`
   (24 h, 150) then `menu_ai_image:global` (24 h, 1000). Refusals return the
   limit codes with `remaining` so the UI can say "te quedan N hoy". The
   global counter reuses the WhatsApp ops-email-at-80% pattern.
...
6. Returns `{ jobId, attempt, remainingToday }`.
...
with a resolved `imageUrl`, plus `attemptCount`. Manager-or-above only.
...
- Caps as above, enforced at `startGeneration`; metered from `usage.cost`.
  At ~$0.04/image (OpenRouter docs example), 150/day/restaurant is ≤ $6.
...
- `ItemImageManager` gains **Generar con IA** beside _Subir imagen_ and the
  ...
  and a retry. `remainingToday` from `startGeneration` is shown once it is
  under 20.
...
- `convex-test` (`convex/_tests/menuAIImageGen.test.ts`): start (auth,
  ...
  numbering, per-restaurant and global caps), record draft, approve (image
  ...
  gone, approved blobs untouched). `fetch` is stubbed; the suite never calls
  OpenRouter.
