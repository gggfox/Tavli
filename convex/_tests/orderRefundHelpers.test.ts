...
describe("the whole-order key and the per-line key", () => {
	it("never collide on the same payment", () => {
		// Removing a line keys (payment, line); a whole-order cancel keys
		// (payment, order). Sharing a key would make Stripe replay the first
		// refund and silently move no money on the second.
		expect(buildRefundIdempotencyKey(PAYMENT_A, ORDER_A)).not.toBe(
